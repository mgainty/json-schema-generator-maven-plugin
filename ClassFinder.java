...
public class ClassFinder {
    ...
    public static List<String> findClassNames(Path root, Globs globs) throws IOException {
        ...
        //System.out.println("ClassFinder::findClassNames LINE 63 matchedClassNames.addAll(findClassPaths(root,globs).map(path -> ClassUtils.convertClassPathToClassName(path.toString(), root.normalize().toString()))="+
        java.util.Collection<java.nio.file.Path> collection=findClassPaths(root, globs);
        ...
        try
        {
        	//System.out.println("ClassFinder::findClassNames LINE 66 outputStream.map(path -> ClassUtils.convertClassPathToClassName(path.toString(), root.normalize().toString()))="+outputStream.map(path -> ClassUtils.convertClassPathToClassName(path.toString(), root.normalize().toString())));
        	matchedClassNames.addAll(outputStream //findClassPaths(root, globs)
                //.stream()
                .map(path -> ClassUtils.convertClassPathToClassName(path.toString(), root.normalize().toString()))
                .collect(Collectors.toList()));			//for some reason we get IllegalStateException with stream already closed
