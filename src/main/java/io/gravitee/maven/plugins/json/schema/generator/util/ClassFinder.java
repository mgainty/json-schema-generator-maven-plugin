...
public class ClassFinder {
	 ...
	 * - Given the root Path: /root/path/ * - Given the Class Path: /root/path/the/path/to/the/class/Class.class * - Then associated Class name
	 ...
	 occurs */ 
    public static java.util.stream.Stream<java.lang.String> outputStream=null; 
    ...
    { 
       ...
       //
        ...
        try 
        { //
		    System.out.println("ClassFinder::findClassNames LINE 66 outputStream.map(path -> ClassUtils.convertClassPathToClassName(path.toString(),
		    root.normalize().toString()))="+outputStream.map(path -> ClassUtils.convertClassPathToClassName(path.toString(),
		    root.normalize().toString()))); 
            matchedClassNames.addAll(outputStream //findClassPaths(root, globs) //.stream() .map(path ->
		    ClassUtils.convertClassPathToClassName(path.toString(), root.normalize().toString())) .collect(Collectors.toList()));			//for some
		    //reason we get IllegalStateException with stream already closed
		} 
 ...
 //try closing 
            ...
            outputStream.close(); 
  //try to reopen 
            System.out.println("!!! IllegalStateException LINE 91 has been thrown attempting re-initialization of stream"); 
    ...
    /** * Find class Paths from the given root Path that match the given list of globs. * * @param root  the root Path from which start searching
	 * @param globs the glob list to taking into account during path matching * @return a list of Paths that match the given list of globs from the
	 root Path * @throws IOException if an I/O occurs */ 
     private static List<Path> findClassPaths(Path root, Globs globs) throws IOException 
     {
         ...
         System.out.println("ClassFinder::findClassPaths LINE 77 new GlobsMatchingClassFileVisitor(globs, matchedPaths)="+new GlobsMatchingClassFileVisitor(globs, matchedPaths)); 
    ...
    {
        /** * The associated GlobPathMatchers */ private final GlobPathMatchers globPathMatchers;

        /** * Path list result */ private final List<Path> matchedPaths;

        /** * Create a new instance based on the given included/excluded globs. Results will be stored into the given mactchedPaths 
         * * @param globs        
                    wrapper to the included/excluded globs configuration * @param matchedPaths the structure to store results */ 
      public GlobsMatchingClassFileVisitor(Globs globs, List<Path> matchedPaths) 
      { 
          System.out.println( "ClassFinder::GlobsMatchingClassFileVisitor(globs="+globs); this.globPathMatchers = new GlobPathMatchers(globs); 
        ...
        /** * Visit the given file and test it against the current globPathMatchers 
          * * @param path           the path to test against the current
		      globPathMatchers 
        ...
        { // If Path is not a class file then continue 
           ...
           { 
               ...
               { 
                   matchedPaths.add(path); return FileVisitResult.CONTINUE; 
               }
           }
           return FileVisitResult.CONTINUE;
        }
        ...
        { 
            ...
            /** * Create a new instance based on the given Globs 
			 ...
			 public GlobPathMatchers(Globs globs) 
             {
                 ...
                 System.out.println("ClassFinder::GlobPathMatchers(Globs) LINE 170 globs.getIncludes().map(include ->FileSystems.getDefault().getPathMatcher('glob:' + formatGlob(include))"+globs.getIncludes().stream().map(include -> FileSystems.getDefault().getPathMatcher("glob:" + formatGlob(include)))); includedMatchers.addAll(globs.getIncludes() .stream() .map(include ->
			     FileSystems.getDefault().getPathMatcher("glob:" + formatGlob(include))) .collect(Collectors.toList()) ); 
            ...
            /** * @return the list of included PathMatchers */ public List<PathMatcher> getIncludedMatchers() { return includedMatchers; }

            /** * @return the list of excluded PathMatchers */ public List<PathMatcher> getExcludedMatchers() { return excludedMatchers; } }
